...
int main()
{
    ...
    std::unordered_multiset<int> int_mset = {10, 20, 10, 30, 10, 20, 30};

    print_multiset(int_mset);

    std::cout << "Total size of this unordered multiset is: " << int_mset.size() << "\n\n";
    ...
    */
    ...
    std::cout << "Set before int_mset.erase(10):\n";
    print_multiset(int_mset);
    std::cout << "Number of 10s before int_mset.erase(10): ";
    std::cout << int_mset.count(10) << "\n\n";

    int_mset.erase(10);
    std::cout << "Set after int_mset.erase(10):\n";
    print_multiset(int_mset);
    std::cout << "Number of 10s after int_mset.erase(10): ";
    std::cout << int_mset.count(10) << "\n\n";

    std::cout << "Using an iterator to erase an element - 20\n";
    std::cout << "Number of 20s before int_mset.erase(iterator): ";
    std::cout << int_mset.count(20) << "\n\n";
    auto it = int_mset.find(20); // returns iterator to the first occurence of the argument
    if (it != int_mset.end())
    {
        int_mset.erase(it);
    }
    print_multiset(int_mset);
    std::cout << "Number of 20s after int_mset.erase(iterator): ";
    std::cout << int_mset.count(20);
    return 0;
}